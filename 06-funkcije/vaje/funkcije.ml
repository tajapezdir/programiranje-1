...
let rec zip list1 list2 =
  ...
  | [] -> (
    match list2 with
    | [] -> []
    | y :: ys -> failwith "ma perche"
  )
  | x :: xs -> (
    match list2 with
    | [] -> failwith "u dumb bitch"
    | y :: ys -> (x, y) :: (zip xs ys)
  )

let rec boljsi_zip = function
  | [], [] ->  []
  | [], _ | _, [] -> failwith "not gud"
...
let rec unzip list1 = 
  let rec unzip_aux list1 acc1 acc2 =
    match list1 with
    | [] -> (acc1, acc2)
    | (prvi, drugi) :: ostanek -> unzip_aux ostanek (acc1 @ [prvi]) (acc2 @ [drugi])
  in 
  unzip_aux list1 [] []

let rec unzip_hitra = function
  | [] -> ([], [])
  | (x, y) :: tl -> let (list1, list2) = unzip_hitra tl in (x :: list1, y :: list2)
...
let rec apply_sequence f x n =
  let rec sequence_aux acc x n =
    match n with
    | 0 -> reverse (x :: acc)
    | _ -> sequence_aux (x :: acc) (f x) (n - 1)
  in
  sequence_aux [] x n 
