...
(* let get_field i j grid =
get_index j (get_index i grid) *)

let get_field (row_i : index) (col_i : index) grid = 
  get_index col_i (get_index row_i grid)

(* let set_field i j x grid = 
  let old = get_index i grid in
  let newr = set_index j x old in
  set_index i newr grid *)
...
(* let is_full_grid (r1, r2, r3) =
  if is_full_row r1 && 
    is_full_row r2 &&
    is_full_row r3 then true
  else false *)
...
let show_state = function
  ...
  | GameOver {result; final_grid} ->
      let winner_message = "bravo, zmagali ste!" in
