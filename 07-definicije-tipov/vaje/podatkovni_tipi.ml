...
let euro_to_dollar (Euro x) = Dollar (x *. 1.22)

let dollar_to_euro (Dollar x) = Euro (x *. 0.82)
...
type currency =
  | Yen of float
  | Pound of float
  | SvedskaKrona of float
  | SvicarskiFrank of float


let to_pound = function
  | Yen x -> Pound (x *. 0.0072)
  | Pound x -> Pound x
  | SvedskaKrona x -> Pound (x *. 0.091)
  | _ -> failwith "Za to enoto ne poznam pretvorbe."
...
let testni_primer = Int(5, Bool(true, Bool(false, Int(7, Nil))))
(*----------------------------------------------------------------------------*]
...
let reverse_list list =
  let rec reverse_aux acc = function
    | [] -> acc
    | x :: xs -> reverse_aux (acc @ [x]) xs
  in
  reverse_aux [] list


let rec intbool_separate ib_list =
  let rec seperate_aux acc1 acc2 = function
    | Nil -> acc1, acc2
    | Int(x, xs) -> seperate_aux (x :: acc1) acc2 xs
    | Bool(x, xs) -> seperate_aux acc1 (x :: acc2) xs
  in
  seperate_aux [] [] ib_list
